...
textData.floran = {
	...
	-- welcome = "^green;Uzh gu'ith aal vozh izh so'ol\nNith vol den'uub den'ek [(scramble)10] GLUTH!^red;\n[(instant)          ][(scramble)1] VERITH [(scramble)1]\n[(instant)     ][(scramble)1] IBN'AAD [(scramble)1]\n[(instant)               ][(scramble)1] IZH OTH [(scramble)1]",
	
	-- welcome = "aaaaaaaaaazazazazaza [(function)func]azazazazazazazazaaaaaaaaaa",
...
function init()
	...
	textData.stationType = "scientific"
	...
	widget.setImage("talkerImage", "/zz/scripts/spaceStation/t"..textData.currentRace..".png:talk.0")
...
function update()
	local sound = "/zz/scripts/spaceStation/chatterGeneric.ogg"
	...
	if textData.textPause <= 0 and not textData.isFinished then
		if textData.talkFrameCooldown <= 0 then
			...
			widget.setImage("talkerImage", "/zz/scripts/spaceStation/t"..textData.currentRace..".png:talk."..tostring(textData.currentTalkFrame))
			textData.talkFrameCooldown = textData[textData.currentRace].talkTicksPerFrame
		else
	...
	elseif textData.isFinished then
		if textData.blinkCooldown <= 0 then
			if textData.blinkFrameCooldown <= 0 then
				widget.setImage("talkerImage", "/zz/scripts/spaceStation/t"..textData.currentRace..".png:blink."..tostring(textData.currentBlinkFrame))
...
function populateGoodsList(forced)
	...
	if forced or not textData.goodsListPopulated then
		...
		for i = 1, #stationGoods do
			...
			if buyRate >= 1.5 then
				widget.setImage(listItem..".buyRate", "/zz/scripts/spaceStation/tradeRate.png:-2")
			elseif buyRate >= 1.2 then
				widget.setImage(listItem..".buyRate", "/zz/scripts/spaceStation/tradeRate.png:-1")
			elseif buyRate >= 0.9 then
				widget.setImage(listItem..".buyRate", "/zz/scripts/spaceStation/tradeRate.png:0")
			...
			if sellRate >= 1.5 then
				widget.setImage(listItem..".sellRate", "/zz/scripts/spaceStation/tradeRate.png:2")
			elseif sellRate >= 1.2 then
				widget.setImage(listItem..".sellRate", "/zz/scripts/spaceStation/tradeRate.png:1")
			elseif sellRate >= 0.9 then
				widget.setImage(listItem..".sellRate", "/zz/scripts/spaceStation/tradeRate.png:0")
		...
		for i, data in ipairs(stationSpecials[textData.stationType]) do
			if not data[5] then -- check if this option was already added
				...
				if secondData then
					...
					widget.setImage(listItem..".background", "/zz/scripts/spaceStation/listSpecialScienceHalf.png")
...
function specialSelected()
	...
	if listItem then
		...
		if textData.stationType == "medical" then
			...
			writerInit(textData, stationSpecials[textData.stationType][stationSpecials.selected.index][4])
		end
	else
		widget.setButtonEnabled("button1", false)
