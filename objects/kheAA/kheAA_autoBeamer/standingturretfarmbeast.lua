...
function fireState(targetId)
  ...
  while true do
	...
	if not world.callScriptedEntity(targetId,"isMonsterHarvestable") then break end
    ...
    if math.abs(util.angleDiff(targetAngle, rotation)) < maxFireAngle then
		--if world.callScriptedEntity(targetId,"isMonsterHarvestable") then
		  fire(targetId)
		--end
    end
    coroutine.yield()
...
function autoFire(target)
	...
	while true do
		...
		if world.callScriptedEntity(target,"isMonsterHarvestable") then
...
function findTarget()
	...
	return util.find(nearEntities,
		function(entityId)
			...
			if world.callScriptedEntity(entityId,"isMonsterHarvestable") then
			--if world.callScriptedEntity(entityId,"config.getParameter","behavior") == "farmable" then
				return true
			end
			return false
		end
