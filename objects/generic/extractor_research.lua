...
local deltaTime=0
local rarityMult={common=1.0, uncommon=1.15, rare=1.25, legendary=1.45,essential=1.45}
	...
	if storage.timer <= 0 then
		if storage.output then
			...
			if (not items[1]) then
				...
				--sb.logInfo("itemData: %s, rVal: %s, timerValue: %s, eVal: %s",itemData,rVal,timerValue,eVal)
				rVal=math.max(math.floor(rVal),1)
				eVal=math.floor(eVal)
				--sb.logInfo("rVal: %s, eVal: %s",rVal,eVal)
				storage.output={research=rVal,essence=eVal}
				
				storage.timer = timerValue
