...
function update(dt)
	...
	if #ents > 0 and world.entityName(ents[1]) == "industrialcentrifuge" then
		...
		if storage.combsProcessed.count and storage.combsProcessed.count >= combsPerJar and (( world.containerConsume(entity.id(), { name= "bottle", count = 1, data={}}) == true ) or ( world.containerConsume(entity.id(), { name= "goldensand", count = 2, data={}}) == true )) then
