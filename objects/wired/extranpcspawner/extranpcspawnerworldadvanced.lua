...
function spawn()
	...
	while attempts > 0 do
		...
		if not self.outOfSight or not world.isVisibleToPlayer({spawnPosition[1] - 3, spawnPosition[2] - 3, spawnPosition[1] + 3, spawnPosition[2] + 3}) then
			...
			local speciescommonrarity = self.speciescommonrarity -- default 15
			local speciesuncommonrarity = self.speciesuncommonrarity -- default 6
			local speciesrarerarity = self.speciesrarerarity -- default 1
			local specieschooser = math.random(1, speciescommonrarity)
			if specieschooser >= speciesuncommonrarity then
				npcSpecies = util.randomFromList(self.npcSpeciesCommon)
			elseif specieschooser < speciesuncommonrarity and specieschooser > speciesrarerarity then
				npcSpecies = util.randomFromList(self.npcSpeciesUncommon)
			else
				npcSpecies = util.randomFromList(self.npcSpeciesRare)
			end
