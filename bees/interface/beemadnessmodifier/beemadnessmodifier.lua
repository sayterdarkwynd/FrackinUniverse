...
function populateItemList(forceRepop)
	local upgradeableQueens = player.itemsWithTag("queen")


	for i = 1, #upgradeableQueens do
		local monkeys=deepSizeOf(upgradeableQueens[i])
		if monkeys <=250 then
			upgradeableQueens[i].count = 1
			local itemLevel
			if upgradeableQueens[i].parameters.level then
				itemLevel = upgradeableQueens[i].parameters.level
			else
				local itemConfig = root.itemConfig(upgradeableQueens[i])
			...
			table.insert(upgradableItems,{itemData=upgradeableQueens[i],itemType="queen",itemLevel=itemLevel})
		end
	end
