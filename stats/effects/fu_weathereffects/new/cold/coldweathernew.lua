...
function init()
if (status.stat("iceResistance",0)  >= 1.0) or status.statPositive("biomecoldImmunity") or status.statPositive("ffextremecoldImmunity") or world.type()=="unknown" then
  effect.expire()
end
      ...
      if status.stat("iceResistance",0) < 1.0 then      
	   ...
	   if status.isResource("food") then
	     ...
	     if status.resource("food") >= 2 then
           ...
           self.statedit = status.stat("iceResistance",0)+ 0.3
           self.statedit2 = status.stat("iceResistance",0)+ 0.3
	         ...
	         airJumpModifier = 0.4 + self.statedit, 
	         speedModifier = 0.2 + self.statedit2 
             })   
      end  
      --breath
