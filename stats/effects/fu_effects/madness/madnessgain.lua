...
function update(dt)
	...
	if self.penaltyTimer >= 1 then
		math.max(0,self.penaltyAmount=self.penaltyAmount-(self.penaltyRate*self.penaltyTimer))
		self.penaltyTimer=0.0
	end

	if (math.abs(mcontroller.xVelocity()) < 5) and (math.abs(mcontroller.yVelocity()) < 5) then
		self.randVal = (math.floor(((math.random(1,4-afkLevel())+self.baseVal)*(1.0 - status.stat("mentalProtection")))*self.valBonus)) * self.penaltyAmount
