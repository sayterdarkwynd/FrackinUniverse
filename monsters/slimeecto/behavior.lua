...
function die()
  local size = config.getParameter("poSize")
    ...
    if size == "medium" then
      local entityId = world.spawnMonster("ectomicroslime", monster.toAbsolutePosition({ -1, 4 }), { level = monster.level() })
      world.callScriptedEntity(entityId, "setSpawnDirection", -1)

      entityId = world.spawnMonster("ectomicroslime", monster.toAbsolutePosition({ 1, 4 }), { level = monster.level() })
      world.callScriptedEntity(entityId, "setSpawnDirection", 1)
    elseif size == "large" then
      local entityId = world.spawnMonster("ectomicroslime", monster.toAbsolutePosition({ -1, 3 }), { level = monster.level() })
      world.callScriptedEntity(entityId, "setSpawnDirection", -1)

      entityId = world.spawnMonster("ectomicroslime", monster.toAbsolutePosition({ 1, 3 }), { level = monster.level() })
      world.callScriptedEntity(entityId, "setSpawnDirection", 1)
    end
end
