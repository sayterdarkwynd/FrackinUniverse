...
function questStart()
  ...
  if associatedMission then
    ...
    player.playCinematic(config.getParameter("missionUnlockedCinema", "/cinematics/coordinates.cinematic"))
    local associatedMission = config.getParameter("associatedMission")
...
function update(dt)
  if self.radioMessageTimer and config.getParameter("missionRadioMessage") ~= nil then
